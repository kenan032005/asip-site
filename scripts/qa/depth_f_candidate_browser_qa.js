#!/usr/bin/env node
/* DEPTH F candidate browser QA: 13 entities + 18 relations + 3 country pages +
   network, four viewports 1920/1366/768/390. Asserts no console errors, no
   runtime exceptions, no failed/bad requests, no horizontal overflow, maturity
   badges, Facts/ASIP Analysis/Watch partitions. Writes
   qa-artifacts-depth-f/candidate-browser-qa.json */
const http = require("http");
const fs = require("fs");
const path = require("path");
const WebSocket = require("ws");
const PUBLIC = process.env.PUBLIC_BASE || "http://127.0.0.1:4181/intelligence/africa";
const CDP_PORT = Number(process.env.CDP_PORT || 9242);
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-depth-f", "candidate-browser-qa.json");

const ENTITIES = [
  ["sspdf", "SSPDF"], ["splm-io", "SPLM/A-IO"], ["national-salvation-front", "NAS"],
  ["salva-kiir", "Salva Kiir"], ["riek-machar", "Riek Machar"],
  ["is-mozambique", "IS-Mozambique"], ["mozambique-defence-forces", "FADM"],
  ["rwanda-force-mozambique", "RDF Mozambique"], ["samim", "SAMIM"], ["tpdf", "TPDF"],
  ["libyan-national-army", "LNA"], ["gnu-forces", "GNU Forces"], ["isis-libya", "ISIS-Libya"],
];
const RELATIONS = [
  ["splm-io-sspdf-conflict", "SPLM-IO-SSPDF"], ["kiir-sspdf-leads", "Kiir-SSPDF"],
  ["machar-splm-io-leads", "Machar-SPLM-IO"], ["nas-splm-io-allied", "NAS-SPLM-IO"],
  ["is-moz-islamic-state", "ISM-IS State"], ["is-moz-islamic-state2", "RDF-ISM (repaired)"],
  ["fadm-is-moz-hostile", "FADM-ISM"], ["rdf-mozambique-fadm-cooperate", "RDF-FADM"],
  ["samim-fadm-cooperate", "SAMIM-FADM (historical)"], ["is-moz-tanzania-link", "ISM-Tanzania"],
  ["fadm-mozambique-operates", "FADM-Mozambique"], ["is-moz-mozambique-operates", "ISM-Mozambique"],
  ["tanzania-tpdf-is-moz", "TPDF-ISM"], ["tanzania-mozambique-cooperate", "TPDF-FADM"],
  ["tanzania-samim-member", "TPDF-SAMIM (historical)"],
  ["lna-gnu-rivalry", "LNA-GNU"], ["isis-libya-affiliation", "ISIS-Libya-IS"],
  ["isis-libya-lna-conflict", "ISIS-Libya-LNA"],
];
...
const ROUTES = [
  ...
  ["South Sudan country", "country/south-sudan/"], ["Mozambique country", "country/mozambique/"],
  ["Libya country", "country/libya/"],
]
...
async function main() {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.method === "Runtime.consoleAPICalled" && msg.params.type === "warning") events.console.push(msg.params);
  ...
  const report = { artifact: "DEPTHF_CANDIDATE_BROWSER_QA", pages, totals: { pages: pages.length } };
