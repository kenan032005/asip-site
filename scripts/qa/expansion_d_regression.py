#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Expansion D full regression runner.

Discovers ALL intelligence test_*.py files + the 2 EXTRA suites (never a
partial runner), runs each, and writes qa-artifacts-expansion-d/test-results.json.
"""
import glob
import io
import json
...
QA = os.path.join(ROOT, "qa-artifacts-expansion-d")
...
def main():
    ...
    for f in files:
        ...
        results.append({"suite": rel, "rc": rc, "cases": cases, "failed_cases": fails,
                        "ok": rc == 0, "tail": out.strip().splitlines()[-6:] if out.strip() else []})
    ...
    payload = {
        "artifact": "EXPANSION_D_FULL_REGRESSION",
    ...
    with open(os.path.join(QA, "test-results.json"), "w", encoding="utf-8") as f:
