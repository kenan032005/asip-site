#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DEPTH D final stats: post-upgrade entity/relation page stats + pre/post
maturity comparison via git show of baseline (Depth C closed 6f80a15)."""
...
pre_ent, pre_rel = {}, {}
try:
    out = subprocess.run(["git", "show", "6f80a15:data/intelligence/africa/entity_profiles.json"],
    ...
    out2 = subprocess.run(["git", "show", "6f80a15:data/intelligence/africa/relation_profiles.json"],
                          capture_output=True, text=True, encoding="utf-8", cwd=ROOT)
...
out = {
    "artifact": "DEPTHD_UPGRADE_COMPARISON",
    "entity_upgrades": ent_cmp,
    "relation_upgrades": rel_cmp,
    "evidence": {"depthd_imported": len(db_ev),
                 "by_status": {s: sum(1 for e in db_ev if e["verification_status"] == s) for s in ("verified", "partially_verified")}},
    "sources": {"total": len(sources), "depthd_added": 13, "reused": ["deptha-acled-july-2026", "acled-sudan-2025"]},
}
