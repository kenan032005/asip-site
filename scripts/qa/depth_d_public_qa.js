#!/usr/bin/env node
/* DEPTH D public QA (github.io): 6 entities + 8 relations + indexes across
   1920/1366/768/390 with CDN cache convergence retries. Writes
   qa-artifacts-depth-d/public-browser-qa.json */
const http = require("http");
const fs = require("fs");
const path = require("path");
const WebSocket = require("ws");
const PUBLIC = process.env.PUBLIC_BASE || "https://kenan032005.github.io/asip-site/intelligence/africa";
const CDP_PORT = Number(process.env.CDP_PORT || 9239);
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-depth-d", "public-browser-qa.json");

const ENTITIES = [
  ["sudanese-armed-forces", "SAF"],
  ["rapid-support-forces", "RSF"],
  ["abdel-fattah-al-burhan", "Burhan"],
  ["mohamed-hamdan-dagalo", "Hemedti"],
  ["splm-n-al-hilu", "SPLM-N al-Hilu"],
  ["justice-equality-movement", "JEM"],
];
const RELATIONS = [
  ["saf-rsf-war", "SAF-RSF war"],
  ["burhan-saf-leads", "Burhan-SAF"],
  ["dagalo-rsf-leads", "Hemedti-RSF"],
  ["splm-n-saf-conflict", "SPLM-N-SAF"],
  ["jem-saf-conflict", "JEM-SAF (repaired)"],
  ["rsf-darfur-origin", "RSF-JEM (repaired)"],
  ["saf-sudan-operates", "SAF-Sudan"],
  ["rsf-sudan-operates", "RSF-Sudan"],
];
...
async function main() {
  ...
  const report = { artifact: "DEPTHD_PUBLIC_BROWSER_QA", pages, totals: { pages: pages.length } };
