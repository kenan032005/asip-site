#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ASIP-PPT-ENTITY-EXPANSION-A §13 全量回归运行器。

依次运行 scripts/tests/intelligence/ 下全部测试 + 本地路径守卫 + 仓库完整性，
汇总 FAIL_TOTAL，写入 qa-artifacts-expansion-a/test-results.json。
"""
import json
import os
import re
...
OUT = os.path.join(ROOT, "qa-artifacts-expansion-a", "test-results.json")
PY = sys.executable
...
def run_one(path):
    ...
    # 优先使用测试自报的 FAIL=<n> 计数；否则回退到 [FAIL] 标记计数
...
def main():
    label = sys.argv[1] if len(sys.argv) > 1 else "EXPANSION_A_FULL_REGRESSION"
    ...
    # test_no_local_paths.py rewrites this historical QA artifact on every run;
    # restore it so the working tree stays clean (Expansion A scope discipline).
    scan = Path(ROOT) / "qa-artifacts-i3b-fix1c" / "local-path-scan.json"
    if scan.exists():
        git_restore = subprocess.run(["git", "checkout", "--", str(scan)], cwd=ROOT,
                                     capture_output=True, text=True, encoding="utf-8", errors="replace")
        if git_restore.returncode != 0:
            print("WARN: could not restore local-path-scan.json:", git_restore.stderr.strip()[:200])
