#!/usr/bin/env node
/* Retry Dan Na network focus on public with cache convergence */
...
async function main() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    await cdp.send("Page.navigate", { url: PUBLIC + "/network/?focus=actor-dan-na-ambassagou" });
    for (let i = 0; i < 40; i++) {
    ...
    console.log("actor-dan-na-ambassagou attempt", attempt, JSON.stringify(v));
    if (v.nodes > 0) break;
    await sleep(2000);
  }
  ws.close();
