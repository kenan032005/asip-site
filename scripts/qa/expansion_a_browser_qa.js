// Expansion A browser QA — verifies rendering of the 11 new entities, 14 new
// relations, the 5 enriched relation dossiers, the lakurawa network_links
// section, and that the whole site still renders without console errors,
// failed requests, broken anchors or horizontal overflow.
...
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-expansion-a", "browser-qa.json");

const ENTITY_ROUTES = [
  ["Al-Shabaab", "entity/al-shabaab/"],
  ["ISIS-Somalia", "entity/isis-somalia/"],
  ["al-Karrar Office", "entity/al-karrar-office/"],
  ["ADF/ISIS-CA", "entity/adf-isis-ca/"],
  ["SIM", "entity/sudanese-islamic-movement/"],
  ["BBMB", "entity/bbmb/"],
  ["Ahmed Diriye", "entity/ahmed-diriye/"],
  ["Abd al-Qadir Mu'min", "entity/abd-al-qadir-mumin/"],
  ["Abdirahman Fahiye", "entity/abdirahman-fahiye/"],
  ["Seka Musa Baluku", "entity/seka-musa-baluku/"],
  ["Ali Ahmed Karti", "entity/ali-ahmed-karti/"],
  ["Lakurawa (enriched, network_links)", "entity/lakurawa/"],
  ["Ansaru (enriched)", "entity/ansaru/"],
];
const RELATION_ROUTES = [
  ["Shabaab-AQ allegiance", "relation/expa-shabaab-alqaida-allegiance/"],
  ["Shabaab-ISIS-Somalia rivalry", "relation/expa-shabaab-isis-somalia-rivalry/"],
  ["ISIS-Somalia-ISIS branch", "relation/expa-isis-somalia-isis-branch/"],
  ["ISIS-Somalia-al-Karrar", "relation/expa-isis-somalia-karrar-colocated/"],
  ["ADF-ISIS branch", "relation/expa-adf-isis-branch/"],
  ["SIM-BBMB linked", "relation/expa-sim-bbmb-linked/"],
  ["BBMB-SAF aligned", "relation/expa-bbmb-saf-aligned/"],
  ["BBMB-RSF hostile", "relation/expa-bbmb-rsf-hostile/"],
  ["Ansaru-JAS split (enriched R3)", "relation/d1-ansaru-jas-split/"],
  ["Lakurawa-IS Sahel (enriched R3)", "relation/d1-lakurawa-is-sahel-network/"],
  ["Lakurawa-JNIM (enriched R3)", "relation/d1-lakurawa-jnim-cooperation/"],
];
const INDEX_ROUTES = [
...
async function main() {
  ...
  const checkPage = async (label, route, kind) => {
    ...
    const state = await evaluate(`(() => {
      ...
      const aqSec = document.querySelector("#asipAnalysis, .analysis-partition");
      return {
        ...
        has_network_links: secs.includes("网络关联与外部联系"),
        has_analysis: !!aqSec,
  ...
  for (const [label, route] of ENTITY_ROUTES) {
    ...
    if (p && p.state.section_count < 8) problems.push(`${label}: only ${p.state.section_count} sections`);
  }
  const laku = pages.find(x => x.route === "entity/lakurawa/");
  if (laku && !laku.state.has_network_links) problems.push("Lakurawa: network_links section missing");
  ...
  const deepRel = ["relation/expa-shabaab-isis-somalia-rivalry/", "relation/expa-isis-somalia-isis-branch/",
                   "relation/expa-adf-isis-branch/", "relation/expa-sim-bbmb-linked/",
                   "relation/d1-ansaru-jas-split/", "relation/d1-lakurawa-is-sahel-network/",
                   "relation/d1-lakurawa-jnim-cooperation/"];
  for (const route of deepRel) {
    const p = pages.find(x => x.route === route);
    if (p && p.state.timeline_items <= 0) problems.push(`${p.label}: no timeline items`);
  }
  const summary = {
  ...
  fs.writeFileSync(OUT, JSON.stringify({ artifact: "EXPANSION_A_BROWSER_QA", generated_at: new Date().toISOString(), public_base: PUBLIC, viewport: "1366x900", pages, events: { exceptions: events.exceptions, console: events.console, failed: events.failed, bad: events.bad, bad_excluding_favicon: realBad }, summary }, null, 2));
