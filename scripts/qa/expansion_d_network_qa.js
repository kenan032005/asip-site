// Expansion D — network focus QA against local dist.
...
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-expansion-d", "network-qa-results.json");
...
const FOCI = [
  ...
  { key: "ansaroul", focus: "actor-ansarul-islam" },
  { key: "katiba_hanifa", focus: "actor-katiba-hanifa" },
  { key: "isis_sinai", focus: "actor-isis-sinai" },
  { key: "fla", focus: "actor-fla" },
];
...
(async () => {
  ...
  for (const f of FOCI) {
    ...
    const state = await ev(`(function(){
      ...
      var labeled = Array.prototype.filter.call(document.querySelectorAll(".graph-node .node-label"), function(x){ return x.textContent.trim().length > 0; }).length;
      ...
      return { nodes: nodes.length, labeled: labeled, center: center, stats: stats };
    })()`);
    const gates = [];
    ...
    console.log(`[done] focus=${f.key} — ${gates.length - failed.length}/${gates.length} nodes=${state.nodes} labeled=${state.labeled} center=${state.center.slice(0, 16)}`);
  }
  const allGates = results.flatMap((r) => r.gates || []);
  const total = allGates.length, passed = allGates.filter((g) => g.pass).length;
  const summary = {
    NETWORK_QA: (allGates.length && results.every((r) => (r.gates || []).every((g) => g.pass))) ? "PASS" : "FAIL",
    total_gates: total, passed_gates: passed, base: BASE, results,
  };
  fs.writeFileSync(OUT, JSON.stringify(summary, null, 2));
  console.log(`=== EXPANSION D NETWORK QA: ${passed}/${total} ===`);
