#!/usr/bin/env node
/* DEPTH C public QA (github.io): 8 priority entities + 11 priority relations +
   indexes across 1920/1366/768/390. CDN edge cache may serve stale JS on the
   first hit, so each page is retried (up to 2 re-navigations) when a maturity
   badge or partition is missing, using the browser cache to converge to the
   fresh bundle. Writes qa-artifacts-depth-c/public-browser-qa.json */
...
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-depth-c", "public-browser-qa.json");

const ENTITIES = [
  ["dan-na-ambassagou", "Dan Na Ambassagou"],
  ["youssouf-toloba", "Youssouf Toloba"],
  ["dozos-of-macina", "Dozos of Macina"],
  ["amadou-nionson-diarra", "Amadou Nionson Diarra"],
  ["dana-atem", "Dana Atem"],
  ["sidi-ongoiba", "Sidi Ongoiba"],
  ["katiba-serma", "Katiba Serma"],
  ["jafar-dicko", "Jafar Dicko"],
  ["ousmane-dicko", "Ousmane Dicko"],
  ["abou-ghosmane", "Abou Ghosmane"],
];
const RELATIONS = [
  ["d1-dan-na-jnim-conflict", "Dan Na-JNIM"],
  ["d1-dan-na-fama-coop", "Dan Na-FAMa"],
  ["d2-dan-na-toloba-led", "Dan Na-Toloba"],
  ["d2-dana-dan-na-split", "Dana-Dan Na"],
  ["d2-dana-sidi-led", "Dana-Sidi"],
  ["d2-dana-katiba-serma-conflict", "Dana-Katiba Serma"],
  ["d2-dana-ansarul-conflict", "Dana-Ansarul"],
  ["d2-dana-fama-coop", "Dana-FAMa"],
  ["d2-dozos-macina-amadou-led", "Dozos-Amadou"],
  ["d2-dozos-macina-jnim-conflict", "Dozos-JNIM"],
  ["d2-dozos-macina-fama-coop", "Dozos-FAMa"],
  ["d2-katiba-serma-jnim", "Katiba Serma-JNIM"],
  ["d2-jafar-jnim", "Jafar-JNIM"],
  ["d2-ansarul-jafar-led", "Ansarul-Jafar"],
  ["d2-ousmane-jnim", "Ousmane-JNIM"],
  ["d2-ghosmane-jnim", "Ghosmane-JNIM"],
];
...
async function main() {
  ...
  // browser cache ON (default) so retries converge to the fresh bundle;
  // we do NOT disable cache here (edge CDN must still be hit once).
  ...
  async function check(width, label, route, kind) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      navs++;
      // converge: retry if maturity badge or partitions missing on upgraded pages
    ...
    const before2 = { exc: events.exceptions.length, con: events.console.length, fai: events.failed.length, bad: events.bad.length };
    pages.push({
      viewport: width, label, route, url, navs,
      state,
      events: {
        runtime_exceptions: events.exceptions.length - before2.exc,
        console_errors: events.console.length - before2.con,
        failed_requests: events.failed.length - before2.fai,
        bad_responses: events.bad.length - before2.bad,
      },
    });
  }
  ...
  const report = { artifact: "DEPTHB_PUBLIC_BROWSER_QA", pages, totals: { pages: pages.length } };
