#!/usr/bin/env node
/* DEPTH E candidate browser QA: 4 entities + 4 relations + Ethiopia country +
   ...
   qa-artifacts-depth-e/candidate-browser-qa.json */
const http = require("http");
const fs = require("fs");
const path = require("path");
const WebSocket = require("ws");
const PUBLIC = process.env.PUBLIC_BASE || "http://127.0.0.1:4180/intelligence/africa";
const CDP_PORT = Number(process.env.CDP_PORT || 9240);
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-depth-e", "candidate-browser-qa.json");
...
async function main() {
  ...
  const report = { artifact: "DEPTHE_CANDIDATE_BROWSER_QA", pages, totals: { pages: pages.length } };
