...
const OUT = path.join(__dirname, "..", "..", "qa-artifacts-depth-a", "network-density-qa.json");
const FOCI = ["actor-jnim", "actor-is-sahel", "person-amadou-koufa", "actor-katiba-macina", "person-iyad-ag-ghali", "actor-fla", "actor-africa-corps"];
...
async function main() {
  ...
  for (const focus of FOCI) {
    ...
    const r = await evaluate(`(() => {
      ...
      return { focus_before: focusId, node_count: nodes.length, edge_count: edgeCountBefore, duplicate_nodes: dup, candidate: click };
    })()`);
