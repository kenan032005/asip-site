#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""端到端冒烟测试：真实来源发现→详情页→正文提取（不写入数据）。"""
import sys, os, json, time
sys.path.insert(0, "scripts")
sys.path.insert(0, "scripts/collectors")

from registry import SourceRegistry, ArticleDiscoverer
from framework import ContentExtractor, fetch_page

reg = SourceRegistry()
discoverer = ArticleDiscoverer(reg)

# 测试：Tchadinfos RSS → 详情页正文
source = None
for s in reg.all():
    lp = s.get("legacy_payload", {})
    if s.get("source_id") == "chad_tchadinfos":
        source = {
            "source_id": s["source_id"], "source_name": s["source_name"],
            "source_country": "乍得", "source_type": s["source_type"],
            "language": "fr", "discovery_type": "rss",
            "feed_url": lp.get("feed_url", ""), "listing_urls": lp.get("category_urls", []),
            "base_url": lp.get("url", ""), "max_items": 5,
            "extractor_profile": {}, "enabled": True,
        }
        break

print("=== RSS 发现 + 详情页正文提取 (Tchadinfos) ===")
if source:
    arts, errs = discoverer.discover(source)
    print(f"发现: {len(arts)} 条, 错误: {errs[:2]}")
    for d in arts[:3]:
        print(f"  - {d['title'][:50]} | {d['url'][:55]}")
        text, err, status = fetch_page(d["url"])
        if err:
            print(f"    FETCH ERR: {err}")
            continue
        ext = ContentExtractor({}).extract(text, d["url"])
        print(f"    → quality={ext['quality']} words={ext['word_count']} method={ext['method']}")
        print(f"      title={ext['title'][:45]}")
        if ext["body"]:
            print(f"      body={ext['body'][:70]}...")
