# -*- coding: utf-8 -*-
"""为 V2 historical backfill preview 生成历史日报 HTML 页面。"""
import json
...
<style>
  ...
  .badge {{ display: inline-block; background: #dbeafe; color: #1d4ed8; border-radius: 4px; padding: 2px 10px; font-size: 12px; margin-left: 8px; vertical-align: middle; }}
  ...
  .analysis {{ background: #f1f5f9; border-left: 3px solid #2563eb; padding: 10px 14px; border-radius: 0 6px 6px 0; color: #475569; font-size: 13px; }}
</style>
</head>
<body>
<div class="wrap">
  <h1>{title} <span class="badge">历史回溯生成 · Historical Reconstruction</span></h1>
  <div class="meta">报告日期：{date} ｜ 生成模式：historical_backfill ｜ 批次：{batch_id}</div>
  <div class="card">
  ...
  <p class="meta">本报告为历史回溯（Historical Backfill）预览产物，未进入 Production。FALLBACK 表示事实版，不代表 AI 已生成研判。</p>
</div>
</body>
</html>"""
...
def main():
    ...
    for f in sorted(REPORTS.glob("daily_*.json")):
        ...
        facts_html = "".join(
            ...
            for fa in doc["facts"])
        if not facts_html:
            facts_html = '<p class="meta">本日无通过事实门禁的 Master Event。</p>'
        analysis_html = (
            "历史回溯报告为确定性事实版（未调用 AI 分析）。综合研判区域留待正式 AI 契约接入后填充。"
            if doc.get("analysis") is None else esc(json.dumps(doc["analysis"], ensure_ascii=False))
        )
        out.write_text(TEMPLATE.format(
            title="非洲地区社会安全与综合形势日报（历史回溯）", date=day,
            batch_id=BATCH_ID, status_cn="事实版" if doc["status"] == "FALLBACK" else doc["status"],
            status=doc["status"], fact_count=doc["fact_count"], gates_html=gates_html,
            facts_html=facts_html, analysis_html=analysis_html), encoding="utf-8")
        entries.append({
            "report_id": doc["report_id"], "title": "非洲地区社会安全与综合形势日报（历史回溯）",
            "type": "africa_daily", "type_cn": "非洲日报", "status": doc["status"],
            "status_cn": "事实版 · 历史回溯", "period_start": day, "period_end": day,
            "published_at": day, "path": "reports/africa_daily/%s/" % day,
            "country_iso3": None, "is_mock": False, "generation_mode": "historical_backfill",
            "historical_reconstruction": True, "backfill_batch_id": BATCH_ID,
        })
        print("page:", day)

    rip = DIST / "data" / "report_index.json"
    ri = json.loads(rip.read_text(encoding="utf-8")) if rip.exists() else {}
    existing = {r["report_id"] for r in ri.get("reports", [])}
    added = [e for e in entries if e["report_id"] not in existing]
    ri["reports"] = sorted(
        added + ri.get("reports", []),
        key=lambda r: (str(r.get("period_end") or r.get("published_at") or ""), str(r.get("report_id") or "")),
        reverse=True,
    )
    ri["count"] = len(ri["reports"])
    ri["note"] = "V2 historical backfill preview: +%d daily reports (%s)" % (len(added), BATCH_ID)
    rip.parent.mkdir(parents=True, exist_ok=True)
