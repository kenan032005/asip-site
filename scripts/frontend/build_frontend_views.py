...
def build_site_overview(events, pub_events, countries, status, disease_tls,
    ...
    if daily_input:
        latest_daily = {
            ...
            "status": "historical_backfill" if daily_input.get("generation_mode") == "historical_backfill" else "development_sample",
            "status_cn": "事实版 · 历史回溯" if daily_input.get("generation_mode") == "historical_backfill" else "开发样例",
        }
    return {
        "generated_at": now,
