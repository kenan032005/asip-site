#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DEPTH C import: source reuse (all 4 mapped to existing), 5 semantic
cleanups, 10 entity upgrades, 16 relation upgrades, 14 evidence, catalog
metrics, alias/graph index. Authority: ASIP_Depth_C_Central_Mali_Content_Pack.json.
...
PACK = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("C:/Users/kenan/Downloads/ASIP_Depth_C_Central_Mali_Content_Pack.json")
...
def main():
    ...
    # ---------- source dedupe (4 candidates, all expected to reuse) ----------
    ...
    dump(QA / "source-mapping.json", {"artifact": "DEPTHC_SOURCE_MAPPING", "mapping": source_map, "blocked_source_metadata": []})
    ...
    for eu in pack["entity_upgrades"]:
        ...
        # freshness: packet sets explicitly; never auto-upgrade aging -> current
    ...
    for ru in pack["relation_upgrades"]:
        ...
        for k in ("nature", "formation_background", "initial_relationship", "evolution_stages", "drivers",
                  ...
                  "current_structure", "geographic_scope", "operational_role"):
        ...
        relation_upgrade_report.append({"relationship_id": rid, "status": "UPGRADED",
                                        ...
                                        "type_unchanged": r["relationship_type"], "sources": len(new_srcs)})
    ...
    STATUS_MAP = {
        "verified": "verified",
        "verified_with_title_variation": "verified",   # source title variation noted; fact verified
    ...
    for i, cl in enumerate(pack["evidence"], start=1):
        ...
        method = "DEPTH C external research/fact-check channel import"
        claim_type = "fact"
        if pstatus == "verified_with_title_variation":
            method += "; packet status verified_with_title_variation (source title wording differs; fact verified)"
        rec = {
            "evidence_id": f"ev-depthc-{i:03d}", "claim_id": cid,
    ...
    catalog.update({
        "generated_at": TODAY, "generated_by": "scripts/gen/depth_c_import.py (machine computed)",
