#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DEPTH D import: source dedupe (15 candidates, 2 reused), 6 fact/semantic
cleanups, 6 entity upgrades, 8 relation upgrades (incl. 2 count-preserving
repairs: rel-jem-saf-conflict hostile_to->cooperates_with, rel-rsf-darfur-origin
historically_associated_with->fought_against), 15 evidence, catalog metrics,
alias/graph index. Authority: ASIP_Depth_D_Sudan_Content_Pack.json. No research,
no new entities/relations/countries/ontology."""
...
def main():
    ...
    dump(QA / "source-mapping.json", {"artifact": "DEPTHD_SOURCE_MAPPING", "mapping": source_map, "blocked_source_metadata": []})

    ents_by_id = {e["entity_id"]: e for e in entities["entities"]}
    ep_by_id = entity_profiles["profiles"]

    # ============================================================
    # FACT CLEANUP GROUPS (targeted phrase cleanup on existing profiles)
    # ============================================================
    cleanup_report = {}
    # A. stale 2024 framing phrases on SAF/RSF
    stale_phrases = ["控制整个苏丹", "整个苏丹的控制"]
    cleanup_a = {}
    for eid in ("actor-saf", "actor-rsf"):
        pr = ep_by_id.get(eid)
        if not pr:
            continue
        ...
        for k in list(secs.keys()):
            ...
            if isinstance(v, str):
                for ph in stale_phrases:
                    if ph in v:
                        v = v.replace(ph, "控制全国（不准确表述，应以分区控制为准）")
                        changed.append(f"{k}:{ph[:12]}")
                secs[k] = v
        cleanup_a[eid] = changed
    cleanup_report["stale_2024_framing"] = cleanup_a

    # F. atrocity attribution guard (no final conviction language)
    convict_phrases = ["已被定罪", "司法定罪", "法庭已定罪", "被国际法庭定罪"]
    cleanup_f = {}
    for eid in ("actor-rsf", "person-mohamed-hamdan-dagalo"):
        pr = ep_by_id.get(eid)
        if not pr:
            continue
        secs = pr.setdefault("sections", {})
        ...
        for k in list(secs.keys()):
            v = secs[k]
            if isinstance(v, str):
                for ph in convict_phrases:
                    if ph in v:
                        v = v.replace(ph, "处于调查/指控阶段（未经司法终审）")
                        changed.append(f"{k}:{ph}")
                secs[k] = v
        cleanup_f[eid] = changed
    cleanup_report["atrocity_attribution"] = cleanup_f
    ...
    # ============================================================
    # 8 relation upgrades (incl. 2 count-preserving repairs)
    # ============================================================
    repair_report = {}
    ...
    for ru in pack["relation_upgrades"]:
        ...
        if r is None:
            ...
            continue
        # count-preserving repair: type override (keep legacy ID, no new relation)
        if "relationship_type_override" in ru:
            before_type = r["relationship_type"]
            ...
            r["current_status_detail"] = ru.get("current_status_override", r.get("current_status"))
            repair_report[rid] = {"before_type": before_type, "after_type": ru["relationship_type_override"],
                                  "legacy_id_retained": ru.get("legacy_id_retained", True),
    ...
    STATUS_MAP = {
        ...
        "verified_reported_finding": "verified",          # UN investigation finding (attributed, not conviction)
        "verified_reported_draft_findings": "verified",   # Reuters-reported draft UN experts findings
    ...
    for i, cl in enumerate(pack["evidence"], start=1):
        ...
        method = "DEPTH D external research/fact-check channel import"
        claim_type = "fact"
        if pstatus == "verified_reported_finding":
            method += "; packet status verified_reported_finding (UN investigation finding with attribution, not court conviction)"
        if pstatus == "verified_reported_draft_findings":
            method += "; packet status verified_reported_draft_findings (Reuters-reported draft UN experts findings)"
        if pstatus == "analytical_synthesis":
            method += "; packet status analytical_synthesis (ASIP analysis, not a verified fact)"
            claim_type = "analysis"
        rec = {
            "evidence_id": f"ev-depthd-{i:03d}", "claim_id": cid,
    ...
    catalog.update({
        "generated_at": TODAY, "generated_by": "scripts/gen/depth_d_import.py (machine computed)",
