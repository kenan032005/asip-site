#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DEPTH B import: source dedupe, 5 fact-error cleanups (source-of-truth),
8 entity upgrades, 11 relation upgrades (incl. count-preserving malformed
rel-jas-islamic-state-hostile repair), 17 evidence, catalog metrics,
alias/graph index. Authority: ASIP_Depth_B_Lake_Chad_Content_Pack.json.
No research, no new entities/relations/countries/ontology."""
...
PACK = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("C:/Users/kenan/Downloads/ASIP_Depth_B_Lake_Chad_Content_Pack.json")
...
def record_key(u):
    """Return digitallibrary record id (record/<digits>) for normalized dedupe,
    so a UN record landing page and its PDF direct link are treated as one source."""
...
def main():
    ...
    dump(QA / "source-mapping.json", {"artifact": "DEPTHB_SOURCE_MAPPING", "mapping": source_map, "blocked_source_metadata": []})

    ents_by_id = {e["entity_id"]: e for e in entities["entities"]}
    ep_by_id = entity_profiles["profiles"]

    # ============================================================
    # FACT CLEANUP GROUP A: ISWAP leadership (al-Barnawi != Bakura)
    # ============================================================
    a_cleanup = {}
    barnawi_phrases = ["巴库拉（Abu Musab al-Barnawi）", "Abu Musab al-Barnawi（巴库拉）", "巴库拉 (Abu Musab al-Barnawi)", "Abu Musab al-Barnawi 2021年死亡", "al-Barnawi于2021年确认死亡", "于2021年确认死亡"]
    for eid in ("actor-iswap", "actor-jas"):
        pr = ep_by_id.get(eid)
        if not pr:
            continue
        ...
        for key in list(secs.keys()):
            v = secs[key]
            if isinstance(v, str):
                for ph in barnawi_phrases:
                    if ph in v:
                        v = v.replace(ph, "Abu Musab al-Barnawi")
                        changed.append(key)
                secs[key] = v
        a_cleanup[eid] = changed

    # ============================================================
    # FACT CLEANUP GROUP C (count-preserving repair) + B: malformed
    # rel-jas-islamic-state-hostile -> JAS->Islamic State pledge
    # ============================================================
    mal = rels_by_id.get("rel-jas-islamic-state-hostile")
    mal_repair = {}
    if mal:
        before = {
            "target": mal["target_entity_id"], "type": mal["relationship_type"],
            "direction": mal.get("direction"), "time_start": mal.get("time_start"),
            "time_end": mal.get("time_end"), "current_status": mal.get("current_status"),
            "freshness": mal.get("freshness_status"),
        }
        ru = next(x for x in pack["relation_upgrades"] if x["relationship_id"] == "rel-jas-islamic-state-hostile")
        sec = ru["sections"]
        mal["target_entity_id"] = "actor-islamic-state"
        mal["relationship_type"] = "pledged_allegiance_to"
        mal["direction"] = "unidirectional"
        mal["current_status"] = "historical_pledge_recognition_shifted_to_iswap"
        mal["current_status_detail"] = "historical_pledge_recognition_shifted_to_iswap"
        mal["time_start"] = "2015-03-07"
        mal["time_end"] = "2016-08-03"
        mal["start_year"] = 2015
        mal["confidence"] = "high"
        mal["relation_summary"] = sec["relationship_summary"]
        mal["formation_background"] = sec["relationship_summary"]
        mal["why_it_matters"] = sec.get("why_it_matters", "")
        mal["uncertainties"] = sec.get("uncertainties", "")
        mal["claim_valid_as_of"] = "2026-08-09"
        mal["freshness_status"] = "historical"
        mal["record_updated_at"] = TODAY
        mal["last_verified_at"] = TODAY
        mal["current_status_verified_at"] = TODAY
        mal["disputed"] = False
        new_srcs = [source_map[sid]["actual_source_id"] for sid in ru.get("source_ids", []) if sid in source_map]
        for sid in new_srcs:
            if sid not in mal.setdefault("source_refs", []):
                mal["source_refs"].append(sid)
        mal_repair = {"before": before, "after": {"target": mal["target_entity_id"], "type": mal["relationship_type"],
                                                  "direction": mal["direction"], "time_start": mal["time_start"],
                                                  "time_end": mal["time_end"], "current_status": mal["current_status"],
                                                  "freshness": mal["freshness_status"]}}
    report["steps"]["malformed_relation_repair"] = mal_repair

    # ============================================================
    # FACT CLEANUP GROUP E: Cameroon relations remove un-jnim-2018
    # ============================================================
    cam_src_fix = {}
    for rid, packet_sids in (("rel-cameroon-army-jas", ["depthb-nctc-boko-haram-2025-06", "depthb-acled-lake-chad-q1-2026"]),
                             ("rel-cameroon-army-iswap", ["depthb-nctc-iswap-2025-04", "depthb-acled-lake-chad-q1-2026", "depthb-iss-mnjtf-2026-05-14"])):
        r = rels_by_id.get(rid)
        if not r:
            continue
        old = list(r.get("source_refs", []))
        if "un-jnim-2018" in r.get("source_refs", []):
            r["source_refs"].remove("un-jnim-2018")
        new_srcs = [source_map[sid]["actual_source_id"] for sid in packet_sids if sid in source_map]
        for sid in new_srcs:
            if sid not in r["source_refs"]:
                r["source_refs"].append(sid)
        r["record_updated_at"] = TODAY
        r["last_verified_at"] = TODAY
        cam_src_fix[rid] = {"before": old, "after": r["source_refs"]}
    report["steps"]["cameroon_source_cleanup"] = cam_src_fix
    ...
    for ru in pack["relation_upgrades"]:
        ...
        for k in ("nature", "formation_background", "initial_relationship", "evolution_stages", "drivers", "constraints", "third_party_effects", "personnel_flows", "cooperation_dimensions", "continuities", "differences", "current_assessment", "current_status", "why_it_matters", "uncertainties", "asip_analysis", "watch_indicators", "organizational_balance", "role", "current_structure", "geographic_scope", "operational_role"):
        ...
        # mechanical timeline from evolution_stages when the relation has none
        ...
        relation_upgrade_report.append({"relationship_id": rid, "status": "UPGRADED", "target_maturity": ru["target_relation_maturity"], "type_unchanged": r["relationship_type"], "sources": len(new_srcs)})
    ...
    # ============================================================
    # 17 evidence (status never auto-upgraded)
    # ============================================================
    STATUS_MAP = {
        "verified": "verified",
        "verified_estimate": "verified",                      # figure is an estimate; keep estimate qualifier
        "verified_reported_with_uncertainty": "partially_verified",
        "verified_with_scope_limit": "verified",              # scope limit kept in claim text
        "analytical_synthesis": "partially_verified",         # analysis, NOT verified fact
    }
    ev_import = []
    for i, cl in enumerate(pack["evidence"], start=1):
        ...
        method = "DEPTH B external research/fact-check channel import"
        claim_type = "fact"
        if pstatus == "verified_estimate":
            method += "; packet status verified_estimate (figure is an estimate, not exact)"
            claim_type = "estimate"
        if pstatus == "verified_reported_with_uncertainty":
            method += "; packet status verified_reported_with_uncertainty (reported leader, member-state divergence)"
        if pstatus == "verified_with_scope_limit":
            method += "; packet status verified_with_scope_limit (scope-limited claim)"
        if pstatus == "analytical_synthesis":
            method += "; packet status analytical_synthesis (ASIP analysis, not a verified fact)"
            claim_type = "analysis"
        rec = {
            "evidence_id": f"ev-depthb-{i:03d}", "claim_id": cid,
        ...
        ev_import.append({"claim_id": cid, "status": "IMPORTED", "evidence_id": rec["evidence_id"], "verification_status": status, "packet_status": pstatus})
    ...
    catalog.update({
        "generated_at": TODAY, "generated_by": "scripts/gen/depth_b_import.py (machine computed)",
    ...
    print(json.dumps({
        ...
        "malformed_repair": mal_repair,
        "cameroon_src_fix": cam_src_fix,
        "a_cleanup": a_cleanup,
