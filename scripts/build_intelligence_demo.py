...
def build_intelligence_demo(dist_root):
    ...
    template = (DEMO_SOURCE / "entity" / "_template.html").read_text(encoding="utf-8")
    for entity in entities:
        entity_dir = target / "entity" / entity["slug"]
        entity_dir.mkdir(parents=True, exist_ok=True)
        (entity_dir / "index.html").write_text(template.replace("__ENTITY_SLUG__", entity["slug"]), encoding="utf-8")
    ...
    print(f"  intelligence demo: {len(entities)} entity routes + network + data")
