...
def main():
    ...
    if not (30 <= len(entities) <= 120): fail(f"entities={len(entities)} outside 30-120 (Expansion B: 94)")
    if not (60 <= len(rels) <= 260): fail(f"relations={len(rels)} outside 60-260 (Expansion B: 181)")
