...
def main():
    ...
    if not (30 <= len(entities) <= 95): fail(f"entities={len(entities)} outside 30-95 (I3-D1: 61; Expansion A: 83)")
    if not (60 <= len(rels) <= 220): fail(f"relations={len(rels)} outside 60-220 (I3-D1: 121; Expansion A: 164)")
