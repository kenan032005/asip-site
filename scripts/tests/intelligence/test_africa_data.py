...
def main():
    ...
    if not (30 <= len(entities) <= 140): fail(f"entities={len(entities)} outside 30-140 (Expansion C: 102)")
    if not (60 <= len(rels) <= 300): fail(f"relations={len(rels)} outside 60-300 (Expansion C: 192)")
