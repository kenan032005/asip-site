...
def test_i3d1_packet_counts():
    ...
    if len(entities) != 102: fail(f"non-country entities={len(entities)} != 102 (83 + 11 Expansion B)")
    if len(rels) != 192: fail(f"relationships={len(rels)} != 192 (164 + 17 Expansion B)")
