...
def main():
    ...
    for rid in deep_rels:
        ...
        if pr.get("relation_maturity"):
            ...
            # DEPTH B: historical R3 relations (freshness=historical) carry full schema
            # but no watch_indicators (nothing to watch on a closed historical pledge);
            # the packet deliberately omits them, so they are not required.
