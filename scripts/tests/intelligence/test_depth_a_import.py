...
def test_depth_a_no_count_expansion():
    ...
    if len(entities) != 72: fail(f"entities={len(entities)} != 72")
    if len(rels) != 150: fail(f"relations={len(rels)} != 150")
...
def test_depth_audit():
    ...
    if len(ea["entities"]) != 72: fail(f"entity audit count {len(ea['entities'])} != 72")
    if len(ra["relations"]) != 150: fail(f"relation audit count {len(ra['relations'])} != 150")
