...
def test_depth_a_no_count_expansion():
    ...
    if len(entities) != 83: fail(f"entities={len(entities)} != 83 (baseline 72 + Expansion A 11)")
    if len(rels) != 164: fail(f"relations={len(rels)} != 164 (baseline 150 + Expansion A 14)")
