...
def main():
    ...
    check("preview URL recorded", bool(url), str(url))
    if url:
        ...
        try:
            ...
            check("preview URL reachable (HTTP 200)", r.status == 200, str(r.status))
            check("preview serves HTML", "html" in body.lower() or "doctype" in body.lower())
        except Exception as ex:
            check("preview URL reachable", False, str(ex)[:120])
